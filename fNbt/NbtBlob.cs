...
namespace fNbt {
    ...
    /// <remarks> Unlike <see cref="NbtFile"/>, blobs are never compressed at this layer, may (per flavor)
    /// have unnamed or non-compound roots, and reads stop exactly at the end of one document, leaving
    /// any trailing bytes in place. Reads are generous: any root tag type is accepted for every flavor.
    /// Writes enforce the flavor's root rules. </remarks>
    public static class NbtBlob {
        ...
        /// <exception cref="NbtFormatException"> If the document is malformed, nested more than 512 levels deep,
        /// or consists of a lone <c>TAG_End</c> byte (use <see cref="TryReadTag(Stream,NbtFlavor,out NbtTag)"/>
        /// to accept absent documents). </exception>
        public static NbtTag ReadTag(Stream stream, NbtFlavor flavor) {
            return ReadTagInternal(stream, flavor, null);
        }


        /// <summary> Reads one NBT document from the given stream, requiring a specific root tag type.
        /// The stream is left positioned exactly past the end of the document. </summary>
        /// <param name="stream"> Stream to read from. Does not need to be seekable. </param>
        /// <param name="flavor"> Encoding to read the document with. </param>
        ...
        /// <exception cref="EndOfStreamException"> If the stream ends before the document does. </exception>
        /// <exception cref="NbtFormatException"> If the document is malformed, nested more than 512 levels deep,
        /// or its root tag type does not match <paramref name="expectedRootType"/>. </exception>
        public static NbtTag ReadTag(Stream stream, NbtFlavor flavor, NbtTagType expectedRootType) {
            ...
            return ReadTagInternal(stream, flavor, expectedRootType);
        }
        ...
        /// <exception cref="NbtFormatException"> If the document is malformed, nested more than 512 levels deep,
        /// or consists of a lone <c>TAG_End</c> byte. </exception>
        public static NbtTag ReadTag(byte[] buffer, int index, int length, NbtFlavor flavor, out int bytesConsumed) {
            ...
            using (var ms = new MemoryStream(buffer, index, length)) {
                NbtTag tag = ReadTagInternal(ms, flavor, null);
        ...
        /// <exception cref="NbtFormatException"> If the document is malformed or nested more than 512 levels deep. </exception>
        public static bool TryReadTag(Stream stream, NbtFlavor flavor, [NotNullWhen(true)] out NbtTag? tag) {
            ValidateArgs(stream, flavor);
            int firstByte = stream.ReadByte();
            if (firstByte < 0) {
                tag = null;
                return false;
            }
            tag = ParseDocument(stream, flavor, firstByte, null);
            return tag != null;
        }
        ...
        public static bool TryReadTag(byte[] buffer, int index, int length, NbtFlavor flavor,
            ...
            if (buffer == null) throw new ArgumentNullException(nameof(buffer));
            using (var ms = new MemoryStream(buffer, index, length)) {
                bool result = TryReadTag(ms, flavor, out tag);
                bytesConsumed = (int)ms.Position;
                return result;
            }
        }
        ...
        /// <exception cref="NotSupportedException"> <paramref name="flavor"/> is not yet supported. </exception>
        /// <exception cref="EndOfStreamException"> If the stream ends partway through a document. </exception>
        /// <exception cref="NbtFormatException"> If a document is malformed, nested more than 512 levels deep,
        /// or consists of a lone <c>TAG_End</c> byte. </exception>
        public static IEnumerable<NbtTag> ReadConcatenatedTags(Stream stream, NbtFlavor flavor) {
            ValidateArgs(stream, flavor);
            return ReadConcatenatedTagsIterator(stream, flavor);
        }
        ...
        /// if a compound contains unnamed tags; if a list has Unknown list type and no elements;
        /// if a string is too long; or if tags are nested more than 512 levels deep. </exception>
        ...
        /// or <paramref name="tag"/> is <c>null</c> and the flavor requires a compound root. </exception>
        /// <exception cref="NotSupportedException"> <paramref name="flavor"/> is not yet supported;
        /// or the document does not fit in a single array. </exception>
        /// <exception cref="NbtFormatException"> If <paramref name="tag"/> is not a compound and the flavor requires one;
        /// if a compound contains unnamed tags; if a list has Unknown list type and no elements;
        /// if a string is too long; or if tags are nested more than 512 levels deep. </exception>
        public static byte[] WriteTag(NbtTag? tag, NbtFlavor flavor) {
            ...
            WriteTag(tag, new MemoryStream(result, 0, result.Length, true), flavor);
            return result;
        }


        static NbtTag ReadTagInternal(Stream stream, NbtFlavor flavor, NbtTagType? expectedRootType) {
            ValidateArgs(stream, flavor);
            int firstByte = stream.ReadByte();
            if (firstByte < 0) {
                throw new EndOfStreamException();
            }
            NbtTag? tag = ParseDocument(stream, flavor, firstByte, expectedRootType);
            if (tag == null) {
                throw new NbtFormatException(
        ...
        static NbtTag? ParseDocument(Stream stream, NbtFlavor flavor, int typeByte, NbtTagType? expectedRootType) {
        ...
        static IEnumerable<NbtTag> ReadConcatenatedTagsIterator(Stream stream, NbtFlavor flavor) {
            while (true) {
                int firstByte = stream.ReadByte();
                if (firstByte < 0) yield break;
                NbtTag? tag = ParseDocument(stream, flavor, firstByte, null);
                if (tag == null) {
        ...
        static void ValidateArgs(Stream stream, NbtFlavor flavor) {
            if (stream == null) throw new ArgumentNullException(nameof(stream));
            if (flavor == null) throw new ArgumentNullException(nameof(flavor));
            if (flavor.UsesVarInts) {
                throw new NotSupportedException("The " + flavor.Name + " flavor is not supported yet.");
            }
        }
    }
}
