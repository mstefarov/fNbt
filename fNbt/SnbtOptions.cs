...
namespace fNbt {
    /// <summary> Settings for SNBT (stringified NBT) text.
    /// Every call reads the values it needs when it starts,
    /// so changing an instance later does not affect a call in progress.
    /// The static <c>Default*</c> properties affect new instances and the overloads that
